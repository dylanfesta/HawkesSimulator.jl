...
function test_with_Hawkes_2pop_withrecorder(γ::Real,W::Matrix{R},h::Vector{R};
    τe::Real=0.5,τi::Real=0.3) where R

  Tstart_rec = 20.0
  Tend_rec = Tstart_rec + 60.0 * 60.0
  ...
  nrec = 500_000
  rec = H.RecFullTrain(nrec,2;Tstart=Tstart_rec,Tend=Tend_rec)
  netw = H.RecurrentNetworkExpKernel((pop_e,pop_i),(rec,))

  t_end=run_simulation!(netw,nspikes)
  
  recc = H.get_content(rec)
  rates_e = H.numerical_rates(recc,1,t_end;pop_idx=1,Tstart=recc.Tstart)[1]
  rates_i = H.numerical_rates(recc,1,t_end;pop_idx=2,Tstart=recc.Tstart)[1]
  @info "t_end is $t_end seconds or $(t_end/60.0) minutes"
...
test_with_Hawkes_2pop_withrecorder(0.001,wtest,htest;τe=τtest[1],τi=τtest[2]);

##

@time test_with_Hawkes_2pop_withrecorder(1.0,wtest,htest;τe=τtest[1],τi=τtest[2]);
@time test_with_Hawkes_2pop_withrecorder(10.0,wtest,htest;τe=τtest[1],τi=τtest[2]);