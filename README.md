# Hawkes Processes Simulator (HawkesSimulator.jl)

| **Documentation** | **AI documentation**         | **Build Status**     |
|:------------------|:-----------------------------|:----------------------|
| [![Dev](https://img.shields.io/badge/docs-dev-blue.svg)](https://dylanfesta.github.io/HawkesSimulator.jl/dev) | [![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/dylanfesta/HawkesSimulator.jl) | [![Build Status](https://github.com/dylanfesta/HawkesSimulator.jl/workflows/CI/badge.svg)](https://github.com/dylanfesta/HawkesSimulator.jl/actions) |

Simulates multi-dimensional Hawkes Processes with a variety or interaction kernels.

> [!WARNING]
> This package is under active development, and mostly undocumented and incomplete. For one-dimensional Hawkes processes, and parameter inference, consider alternatives such as [HawkesProcesses.jl](https://github.com/dm13450/HawkesProcesses.jl).